    ...
    if form.validate_on_submit():
        ...
        if form.Ear_img.data is None:
            earningRow = Earnings(Per_id=per_id[0].id, U_id=current_user.id,
                                  ...
                                  Ear_date=form.Ear_date.data, Ear_comm=form.Ear_comm.data)
        else:
            earningRow = Earnings(Per_id=per_id[0].id, U_id=current_user.id,
                                  Ear_per_name=dict(form.Ear_per_name.choices).get(form.Ear_per_name.data),
                                  Ear_type_name=dict(form.Ear_type_name.choices).get(form.Ear_type_name.data),
...
def list_earning():
    ...
    earnings = Earnings.query.filter_by(U_id=current_user.id).order_by(Earnings.Ear_date.desc()).paginate(page, app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('earnings.list_earning', page=earnings.next_num) if earnings.has_next else None
    prev_url = url_for('earnings.list_earning', page=earnings.prev_num) if earnings.has_prev else None
    return render_template('earning/earn_List.html', viewearn=earnings.items,next_url=next_url,prev_url=prev_url)
